...
1. **객체 생성 (Instantiation)**
    - `@Component`, `@Bean` 등의 방식으로 빈이 생성됨.
        - `@Service`, `@Repository`, `@Controller` 등의 어노테이션을 사용하여 @Component 어노테이션보다 더 구체적인 역할을 명시할 수 있음. -> AOP 관점에서 구체적으로 명시하는 것이 좋음. 어느 것도 해당하지 않으면 `@Component` 어노테이션을 사용.
        - `@Service`, `@Repository`, `@Controller`, `@Component`은 Spring에서 특정 역할을 하는 클래스를 자동으로 빈으로 등록하기 위해 제공하는 스테리오타입 어노테이션입니다.(참고:@Autowired는 이미 등록된 빈을 찾아서 주입 DI 어노테이션이다.)
...
- `의존성 주입 후 초기화 작업 누락`:  빈이 생성된 후 의존성 주입이 완료된 시점에 실행해야 하는 초기화 작업이 누락될 수 있습니다. 의존성이 주입된 후 @PostConstruct 어노테이션을 사용하여 자동으로 초기화 작업을 수행하여 안정성을 높일 수 있습니다.
